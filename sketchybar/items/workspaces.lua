...
aerospace:query_workspaces(function(workspace_info)
	for _, entry in ipairs(workspace_info) do
		...
		local workspace = sbar.add("item", {
			...
			icon = {
				...
				padding_left = 5,
				padding_right = 4,
				string = workspace_index:gsub("^%d+_", ""),
			},
			label = {
				color = colors.with_alpha(colors.white, 0.65),
				drawing = true,
	...
	root:subscribe(
		...
		safe_handler("aerospace_workspace_change", function(env)
	...
	root:subscribe(
		"front_app_switched",
		safe_handler("front_app_switched", function()
	...
	root:subscribe(
		"display_change",
		safe_handler("display_change", function()
